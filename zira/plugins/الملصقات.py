...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"ᯓ 𝗦𝗼𝗠𝟯𝗮 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - معلومـات الملصـق 🎟\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
...
async def cb_sticker(event):
    split = event.pattern_match.group(1) #Code by T.me/SOURCE_SOM3A
...
async def zelzal_gif(event):
    ...
    chat = "@Stickers" #Code by T.me/SOURCE_SOM3A
    zed = await edit_or_reply(event, "**⎉╎جـارِ حـذف الملصـق مـن حزمتـك ...**")
    async with borg.conversation(chat) as conv: #Code by T.me/SOURCE_SOM3A
        try:
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message("/delsticker")
            await conv.get_response()
            await conv.send_file(reply) #Code by T.me/SOURCE_SOM3A
            await conv.get_response()
