...
ZelzalDV_cmd = (
    "[ᯓ 𝙎𝙊𝙐𝙍𝘾𝙀 𝗦𝗼𝗠𝟯𝗮 - اوامــر المطـور المســاعد](t.me/SOURCE_SOM3A) .\n\n"
    ...
    "\n𓆩 [𝗦𝗼𝗠𝟯𝗮 𝗨𝘀𝗲𝗿𝗯𝗼𝘁](t.me/SOURCE_SOM3A) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت سمعه انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗦𝗼𝗠𝟯𝗮 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your SOURCE_SOM3A__.\n"
                )
            else:
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت سمعه انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
...
# Copyright (C) 2022 SOURCE_SOM3A . All Rights Reserved
